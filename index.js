...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++) {
        ...
        div.innerHTML = `   <img class = game-img src = ${games[i].img} />
                            ...
                            <p> Money Pledged: $${games[i].pledged} </p>
...
// use filter or reduce to count the number of unfunded games
    ...
    let len = unfunded.length;


// create a string that explains the number of unfunded games using the ternary operator
const displayString = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently,  ${len == 1 ? `${len} game remains unfunded.`: `${len} games remain unfunded.`} We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
let newDesc = document.createElement('p');
newDesc.innerText = displayString;
...
topGame.innerText = first.name;
firstGameContainer.append(topGame);


// do the same for the runner up item
let runnerUpGame = document.createElement('h2');
runnerUpGame.innerText = second.name;
